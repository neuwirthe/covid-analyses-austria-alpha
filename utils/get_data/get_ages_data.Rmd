...
```{r bmi08}
suppressMessages(
download.file(ages_download_zip_file,
  file.path(local_ages_download_dir,local_ages_file)
)
)
```
```{r}
ages_unzip_dir <-
  file.path(local_ages_download_dir,"ages_files")
create_dir_if_needed(ages_unzip_dir)
```


```{r}
unzip(file.path(local_ages_download_dir,local_ages_file),
      exdir = ages_unzip_dir)
```


```{r}
read_csv2(file.path(ages_unzip_dir,
...
```{r}
ages_raw |>
  select(Time,Bundesland,BundeslandID,AnzEinwohner,
         ends_with("Sum")) |>
  mutate(Datum = Time |> 
           str_sub(1,10) |>
...
read_csv2(file.path(ages_unzip_dir,
                    "CovidFallzahlen.csv")) |>
  mutate(Datum=as.Date(Meldedat,format="%d.%m.%Y",
...
read_csv2(
  file.path(local_ages_download_dir,"ages_files",
    ...
    mutate(Datum=as.Date(Time,format="%d.%m.%Y",
                       tz=Sys.timezone(location=TRUE))) |>
  rename(pop_AGES=AnzEinwohner) |>
  mutate(BundeslandID = floor(GKZ/100)) |>
  rename(BezirkID=GKZ) |>
  select(Datum,Bezirk,BezirkID,BundeslandID,pop_AGES,
         ends_with("Sum")) |>
  rename(cases_AGES=AnzahlFaelleSum) |>
  rename(Tot_AGES = AnzahlTotSum) |>
  rename(Genesen_AGES = AnzahlGeheiltSum) |>
...
if (!keep_download_data) {
  file_delete(
    file.path(local_ages_download_dir,local_ages_file))
  dir_delete(file.path(local_ages_download_dir,"ages_files"))
}
```
